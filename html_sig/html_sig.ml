...
module type S = sig
  ...
  (** {2 Functor} *)
  (*
  (** [concat seq] concatenates all the elements in the sequence
      [seq]. *)
  val concat : 'action html Seq.t -> 'action html
*)
  ...
  type 'a void_element = ?attrs:'a attribute list -> unit -> 'a html
  type 'a void_element' = attrs:'a attribute list -> 'a html
  type 'a escapable_text_element = ?attrs:'a attribute list -> string -> 'a html
  type 'a raw_text_element = ?attrs:'a attribute list -> string -> 'a html
  type 'a normal_element = ?attrs:'a attribute list -> 'a html -> 'a html
  ...
  module A : sig
    ...
    (* table attributes *)
    val colspan : int -> _ attribute
    val align : string -> _ attribute

    (* textarea *)
    val rows : int -> _ attribute

    (* time *)
    val datetime : string -> _ attribute
  end
end
